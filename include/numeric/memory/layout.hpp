...
public:
  ...
  NUMERIC_HOST_DEVICE Layout(Ints... dims) : shape_{static_cast<dim_t>(dims)...} {
    static_assert(sizeof...(dims) == N, "Wrong number of dimensions provided to Layout constructor");
    stride_[N-1] = 1;
    for (dim_t i = 2 ; i <= N ; ++i) {
  ...
  NUMERIC_HOST_DEVICE dim_t shape(size_t idx) const noexcept { return shape_[idx]; }
  ...
  NUMERIC_HOST_DEVICE dim_t &stride(size_t idx) noexcept { return stride_[idx]; }
  NUMERIC_HOST_DEVICE dim_t stride(size_t idx) const noexcept { return stride_[idx]; }
