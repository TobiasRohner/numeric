...
public:
  ...
  NUMERIC_HOST_DEVICE ArrayView(scalar_t *data, const Layout<dim> &layout, MemoryType memory_type = MemoryType::UNKNOWN) : super(data, layout, memory_type) { }
  ...
  NUMERIC_HOST_DEVICE [[nodiscard]] scalar_t &operator()(Idxs... idxs) noexcept {
    return raw()[memory_index(idxs...)];
  }

  NUMERIC_HOST_DEVICE [[nodiscard]] ArrayConstView<Scalar, N> const_view() const noexcept { return *this; }

  using super::memory_type;
  using super::operator();
  using super::raw;
  NUMERIC_HOST_DEVICE [[nodiscard]] scalar_t *raw() noexcept { return const_cast<scalar_t *>(data_); }
  using super::layout;
  using super::shape;
  using super::stride;
  using super::size;
