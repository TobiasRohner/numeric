...
protected:
  ...
  size_t node_tag_to_idx(size_t tag, size_t min_node_tag);
};
...
protected:
  ...
  virtual bool parse_nodes(std::istream &is) override {
    ...
    const size_t min_node_tag = read_value<size_t>(is);
    const size_t max_node_tag = read_value<size_t>(is);
    mesh_.set_vertices(memory::Array<scalar_t, 2>(
        memory::Shape<2>(world_dim_, num_nodes), memory::MemoryType::HOST));
    nodes_are_contiguous_ = (max_node_tag - min_node_tag + 1 == num_nodes);
    for (size_t block = 0; block < num_entity_blocks; ++block) {
      ...
      for (size_t node = 0; node < num_nodes; ++node) {
        ...
        const size_t idx = node_tag_to_idx(tag, min_node_tag);
  ...
  virtual bool parse_elements(std::istream &is) override {
    ...
    for (size_t block = 0; block < num_entity_blocks; ++block) {
      ...
      auto add_elements = [&]<typename Element>() {
        mesh_.template set_elements<Element>(
            mesh::ElementList<Element>(num_elements_in_block));
